...
int main(int argc, char *argv[])
{
  ...
  auto scalit = boost::make_transform_iterator(alphas.begin(), logfirst);
  auto scalend = boost::make_transform_iterator(alphas.end(), logfirst);
  float P = std::accumulate(scalit, scalend, 1.0f);
